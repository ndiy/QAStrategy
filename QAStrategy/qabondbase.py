...
class QAStrategyBondBase(QAStrategyStockBase):
    ...
    def __init__(self, code=['110031'], frequence='day', strategy_id='QA_STRATEGY', risk_check_gap=1, portfolio='cbond',
    ...
    def subscribe_data(self, code, frequence, data_host, data_port, data_user, data_password):
        ...
        self.sub = subscriber_topic(exchange='realtime_bond_{}'.format(
            frequence), host=data_host, port=data_port, user=data_user, password=data_password, routing_key='')
        for item in code:
            self.sub.add_sub(exchange='realtime_bond_{}'.format(
    ...
    def send_order(self,  direction='BUY', offset='OPEN', code=None, price=3925, volume=10, order_id='',):
        ...
        if self.market_type == QA.MARKET_TYPE.STOCK_CN or self.market_type == QA.MARKET_TYPE.BOND_CN:
            """
            在此对于股票/债券的部分做一些转换
            """
...
if __name__ == '__main__':
    QAStrategyBondBase(code=['110031', '123003']).run_sim()
